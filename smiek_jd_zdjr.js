...
!(async () => {
    jd_zdjr_activityUrl = $request.url.replace("/wxTeam/shopInfo","")
    jd_zdjr_activityId = $request.body.replace("activityId=", "")
    
    if (jd_zdjr_activityId != '' && jd_zdjr_activityUrl != ''){
        $.msg(`获取成功\njd_zdjr_activityId=${jd_zdjr_activityId}\njd_zdjr_activityUrl=${jd_zdjr_activityUrl}`)
        msg = `export jd_zdjr_activityId="${jd_zdjr_activityId}"\nexport jd_zdjr_activityUrl="${jd_zdjr_activityUrl}"`
        console.log('开始推送')
        await tgBotNotify(msg)
        console.log('推送结束')
    }else{
        $.msg("获取失败")
    }
    ...
    function tgBotNotify(msg) {
        return new Promise((resolve) => {
          if (TG_BOT_TOKEN && TG_USER_ID) {
            const options = {
              url: `https://${TG_API_HOST}/bot${TG_BOT_TOKEN}/sendMessage`,
              body: `chat_id=${TG_USER_ID}&text=${msg}&disable_web_page_preview=true`,
              headers: {
